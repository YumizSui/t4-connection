...
3. 実装repoの `local/install` を実行する。配置先は `~/.local/share/t4-connection/`、コマンドの入口は `~/.local/bin/`。`Host t4-compute` も登録され、接続時に最新の計算ノードを解決する。接続用の各PCでも実行する。シェル設定でPATHを通し、同名の古い関数・aliasが優先されないことを確認する。既存設定を変更する前にバックアップする。
...
- 各PCで更新した実装repoの `local/install` を再実行する。TSUBAME側は `local/deploy` で更新する。PCの `T4_START_HOURS` / `T4_START_SERVICE` で引数省略時の動作を選べる（未設定なら20時間・sshd）。個人設定は保持し、稼働中サービスの再起動は別の操作として扱う。
- 新しいシェルで `command -v t4-start t4-shell t4-forward t4-ssh-config` と `t4-start --dry-run 1 both` を確認する。
